...
async function run() {
  ...
  console.log(`Debits: Count = ${debits.length}, Sum = ${debitSum}`);
  console.log(`Credits: Count = ${credits.length}, Sum = ${creditSum}`);
  
  console.log("\nCredits parsed:");
  credits.forEach(tx => console.log(`${tx.date} | Amt: ${tx.amount} | Bal: ${tx.balance} | Desc: ${tx.description}`));
}
